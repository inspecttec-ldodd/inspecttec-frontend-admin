...
import { UserGroupListResult, UserGroupSummary, CreateUserGroupRequest, UpdateUserGroupRequest, UserGroupRole } from "@/types/api/user-group";
import { ApiResponse } from "@/types/api/common";
import { useAdminStore } from "@/stores/admin-store";
import { UserSummary } from "@/types/api/user";

export const userGroupService = {
    getUserGroups: async (page = 1, pageSize = 20, search = ""): Promise<UserGroupListResult> => {
        ...
        const response = await apiFetch<ApiResponse<RawGroupResponse>>(`/usergroups?${params}`);
        return {
    ...
    getUserGroupById: async (groupId: string): Promise<UserGroupSummary> => {
        const response = await apiFetch<ApiResponse<UserGroupSummary>>(`/usergroups/${groupId}`);
        return response.result;
    },

    create: async (data: CreateUserGroupRequest): Promise<UserGroupSummary> => {
        const response = await apiFetch<ApiResponse<UserGroupSummary>>(`/usergroups`, {
            method: 'POST',
    ...
    update: async (groupId: string, data: UpdateUserGroupRequest): Promise<UserGroupSummary> => {
        const response = await apiFetch<ApiResponse<UserGroupSummary>>(`/usergroups/${groupId}`, {
            method: 'PUT',
            body: JSON.stringify(data)
        });
    ...
    getGroupRoles: async (groupId: string): Promise<UserGroupRole[]> => {
        ...
        const response = await apiFetch<ApiResponse<{ roles: UserGroupRole[] }>>(`/clients/${clientId}/usergroups/${groupId}/roles`);
    ...
    removeRole: async (groupId: string, roleId: string): Promise<void> => {
        ...
        await apiFetch(`/clients/${clientId}/usergroups/${groupId}/roles/${roleId}`, {
            method: 'DELETE'
        });
    },
