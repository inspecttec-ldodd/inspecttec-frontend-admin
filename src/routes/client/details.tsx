...
import { UserSummary } from '@/types/api/user';
...
import { Users, MapPin, Box, Layers, Calendar } from 'lucide-react';
...
function ClientDetails() {
  ...
  useEffect(() => {
    if (client) {
      form.reset({
        ...
        ownerUserId: client.ownerUserId || '00000000-0000-0000-0000-000000000000',
  ...
  if (!selectedClientId) return <div className="p-8">No client selected.</div>;
      ...
      {client.stats && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{client.stats.activeUsers} / {client.stats.totalUsers}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{client.stats.activeLocations} / {client.stats.totalLocations}</div>
              <p className="text-xs text-muted-foreground">Active / Total Locations</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{client.stats.totalAssets}</div>
              <p className="text-xs text-muted-foreground">In {client.stats.totalAssetGroups} Groups</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{client.stats.lastInspectionDate ? new Date(client.stats.lastInspectionDate).toLocaleDateString() : 'N/A'}</div>
              <p className="text-xs text-muted-foreground">Date of last activity</p>
            </CardContent>
          </Card>
        </div>
      )}
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      ...
                      <SelectContent>
                        {users?.items?.map((user: UserSummary) => (
                    ...
                    <FormDescription>The user who owns this client account.</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-x-8 gap-y-4">
              ...
              <div>
                ...
                <div className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${client.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
              ...
              <div>
                ...
                <div className="font-medium">{client.timeZoneId}</div>
              </div>
            </div>

            <div>
              <Label className="text-muted-foreground mb-1 block">Address</Label>
              <div className="font-medium">
                {client.address1 && <div>{client.address1}</div>}
                {client.address2 && <div>{client.address2}</div>}
                {(client.city || client.state || client.postalCode) && (
                  <div>
                    {client.city}{client.city && client.state ? ', ' : ''}{client.state} {client.postalCode}
                  </div>
                )}
                {!client.address1 && !client.city && <div className="text-muted-foreground">-</div>}
              </div>
            </div>
          </div>
        )}
