...
import { ArrowLeft, Loader2, Plus, Shield, Trash2, User } from "lucide-react";
...
function GroupDetail() {
	...
	const assignRoleMutation = useMutation({
		mutationFn: (roleId: string) =>
			userGroupService.assignRole(groupId, roleId),
		onSuccess: () => {
	...
	const removeRoleMutation = useMutation({
		mutationFn: (roleId: string) =>
			userGroupService.removeRole(groupId, roleId),
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: ["group-roles", groupId],
			});
	...
	const removeUserMutation = useMutation({
		mutationFn: (userId: string) =>
			userGroupService.removeUser(groupId, userId),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["group", groupId] });
			...
			<div className="flex items-center gap-4">
				...
				<div>
					<h2 className="text-3xl font-bold tracking-tight">
						{group.userGroupName} Group Details
					</h2>
			...
			<Tabs defaultValue="roles" className="w-full">
				<TabsList>
					...
					>
						...
						Assigned Roles (
						{group.groupRoles?.length || groupRoles?.length || 0})
					</TabsTrigger>
					<TabsTrigger
						value="members"
						className="flex items-center gap-2"
				...
				<TabsContent value="members" className="space-y-4 mt-4">
					<div className="flex items-center gap-4 p-4 border rounded-lg bg-card">
						...
						<Button
							...
							disabled={
								!selectedUserId || addUserMutation.isPending
							}
						>
