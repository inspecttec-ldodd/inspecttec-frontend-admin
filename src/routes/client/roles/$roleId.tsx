				...
				<div className="flex-1">
					<div className="flex items-baseline gap-4 mb-2">
						<h1 className="text-4xl font-extrabold tracking-tight text-slate-900">
							{role.roleName} Role Permissions
						</h1>
					</div>
						...
						<div className="p-4">
							<div className="grid grid-cols-1 gap-6">
								...
								{(!allPermissions?.categories ||
									allPermissions.categories.length === 0) &&
									allPermissions?.items?.map((perm) => (
										<div
											key={perm.id}
										...
										>
											<Checkbox
												id={perm.id}
												checked={assignedPermissionIds.has(
													perm.id,
												)}
					...
					<div className="border rounded-lg bg-card p-4">
						...
						<div className="space-y-2 text-sm text-muted-foreground">
							<div className="flex flex-col gap-1">
								...
								<span className="font-mono text-xs break-all select-all block bg-muted/30 p-2 rounded">
									{role.id}
								</span>
							</div>
						</div>
