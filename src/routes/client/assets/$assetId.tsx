...
import { AssetType, type UpdateAssetRequest } from "@/types/api/asset";
	...
	function onSubmit(data: AssetFormValues) {
		const updateData: UpdateAssetRequest = {
			...
			assetType: data.assetType
				? (Number(data.assetType) as AssetType)
				: undefined,
		};
		...
		<div className="max-w-4xl mx-auto space-y-6">
			<div className="flex items-center gap-4">
				<Button
					variant="ghost"
					size="icon"
					onClick={() => navigate({ to: "/client/assets" })}
				>
					...
					<div className="space-y-6">
						<div className="grid grid-cols-2 gap-x-8 gap-y-4">
							...
							<div>
								...
								<div className="font-medium">
									{asset.assetGroupName ||
										assetGroups?.items.find(
											(g) => g.id === asset.assetGroupId,
										)?.assetGroupName ||
										"-"}
								</div>
							</div>
							<div>
								...
								<div
									className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${asset.isActive ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
								>
