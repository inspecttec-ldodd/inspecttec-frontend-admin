...
import type { UpdateAssetGroupRequest } from "@/types/api/asset-group";
...
function AssetGroupDetail() {
	...
	const updateMutation = useMutation({
		mutationFn: (data: UpdateAssetGroupRequest) =>
			assetGroupService.updateAssetGroup(groupId, data),
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: ["asset-group", groupId],
			});
			queryClient.invalidateQueries({
				queryKey: ["client-asset-groups"],
			});
		...
		<div className="max-w-4xl mx-auto space-y-6">
			<div className="flex items-center gap-4">
				<Button
					variant="ghost"
					size="icon"
					onClick={() => navigate({ to: "/client/asset-groups" })}
				>
								...
								render={({ field }) => (
									<FormItem className="flex flex-row items-center space-x-2 space-y-0 rounded-md border p-4">
										...
										<div className="space-y-1 leading-none">
											...
											<FormDescription>
												Inactive groups cannot be
												assigned to new assets.
											</FormDescription>
										</div>
									</FormItem>
								)}
					...
					<div className="space-y-6">
						<div className="grid grid-cols-2 gap-x-8 gap-y-4">
							...
							<div>
								...
								<div
									className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${group.isActive ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
								>
