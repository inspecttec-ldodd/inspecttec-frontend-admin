...
function ClientsList() {
    ...
    if (error) {
        return <div className="p-4 text-red-500">Error loading clients: {(error as Error).message}</div>
    }
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-slate-500 mt-1">Manage tenant organizations and subscriptions.</p>
                </div>
                <Link
                    to="/clients/create"
            ...
            <div className="bg-white dark:bg-slate-900 rounded-lg border shadow-sm overflow-hidden">
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {isLoading ? (
                            <tr><td colSpan={7} className="p-8 text-center text-slate-500">Loading clients...</td></tr>
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-xs text-slate-500">{new Date(client.createdDate).toLocaleDateString()}</div>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-6 py-4 text-slate-600">{client.domainName || "-"}</td>
                                <td className="px-6 py-4">
                    ...
                    ${client.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                ...
                                <td className="px-6 py-4 text-slate-600 font-mono">{client.userCount}</td>
                                <td className="px-6 py-4 text-slate-600 font-mono">{client.assetCount}</td>
                                <td className="px-6 py-4 text-slate-600 font-mono">{client.locationCount}</td>
                                <td className="px-6 py-4 text-right">
                                    <button
                                        onClick={() => handleManageClient(client.id, client.name)}
